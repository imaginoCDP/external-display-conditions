...
export class MyExternalDisplayConditions extends ExternalDisplayConditionsLibrary {
  ...
  static MESSAGES = {
    ...
    CONDITION_DESCRIPTION:
      "Only users that fit conditions will see this part of the email.",
  };
  ...
  openExternalDisplayConditionsDialog(
    ...
    // Extra data is empty if loading existing template with condition
    // (ex: the one from template.json)
    console.log(
      "extraData is defined",
      !!this.currentCondition?.extraData,
      this.currentCondition?.extraData
    );
  ...
  getDropdownMarkup(className) {
    return `<select style="${this.styleObjToString(
      MyExternalDisplayConditions.STYLES.select
  ...
  createConditionsPopup() {
    ...
    div.innerHTML = `
        <div id="externalDisplayConditionsPopup"
             style="${this.styleObjToString(
               MyExternalDisplayConditions.STYLES.overlay
             )}; visibility: hidden;"
             class="esdev-app">
            <div style="${this.styleObjToString(
              MyExternalDisplayConditions.STYLES.modal
            )}">
    ...
    this.conditionsPopupElement = document.getElementById(
  ...
  generateHeaderHTML() {
    return `
        <div style="${this.styleObjToString(
          MyExternalDisplayConditions.STYLES.header
        )}">
  ...
  generateContentHTML() {
    ...
    return `
        <div style="${this.styleObjToString(
          MyExternalDisplayConditions.STYLES.content
        )}">
            <!-- Conditions table -->
            ...
            <div style="display: flex; align-items: center; justify-content: space-between;
                ...
                <div style="display: flex; gap: 12px;">
                    <button id="closeConditionsPopup"
                            style="${this.styleObjToString(
                              MyExternalDisplayConditions.STYLES.buttonSecondary
                            )}"
                    ...
                    <button id="applyConditionsAction"
                            style="${this.styleObjToString(
                              MyExternalDisplayConditions.STYLES.buttonPrimary
                            )}"
    ...
    return `
        ...
        <td style="padding: 0 8px 16px 0;">
            <input type="text"
                   ...
                   style="${this.styleObjToString(
                     MyExternalDisplayConditions.STYLES.input
                   )}"
  ...
  updateDeleteActionVisibility() {
    const CSS = MyExternalDisplayConditions.CSS_CLASSES;
    const rows = this.conditionsPopupElement.querySelectorAll(
      `.${CSS.CONDITIONS_TABLE} .${CSS.CONDITION_ROW}`
    );
    if (rows.length > 0) {
      const firstDeleteButton = rows[0].querySelector(
        'button[class*="condition-delete-action"]'
      );
  ...
  showValidationError(message) {
    ...
    const contentDiv = this.conditionsPopupElement.querySelector(
      '[style*="padding: 32px"]'
    );
    if (contentDiv) {
  ...
  hideValidationError() {
    const existingError =
      this.conditionsPopupElement.querySelector(".validation-error");
    if (existingError) {
  ...
  applyConditions() {
    ...
    const concatenation = this.getDropdownValue(
      this.conditionsPopupElement,
      CSS.DROPDOWN_CONCATENATION
    );
    ...
    this.selectConditionsCallback({
      ...
      afterScript: "%/IF%",
      extraData: finalCondition, // just for testing purpose
  ...
  initConditions(appliedCondition) {
    ...
    const table = this.conditionsPopupElement.querySelector(
      `.${CSS.CONDITIONS_TABLE}`
    );
    if (table) {
      table.innerHTML = "";
    ...
    const initialConditions = this.parseAppliedCondition(
      appliedCondition.beforeScript
    );
    ...
    this.setDropdownValue(
      this.conditionsPopupElement,
      CSS.DROPDOWN_CONCATENATION,
      initialConditions.concatenation
    );
  }
