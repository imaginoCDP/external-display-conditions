...
export const CUSTOM_FONT_FAMILY_SELECT_UI_ELEMENT_ID =
...
export class CustomFontFamilySelectUIElement extends UIElement {
  ...
  private originalSelect!: HTMLElement & {
    value: string;
    setUIEAttribute: (name: string, value: any) => void;
  };
  ...
  onRender(container: HTMLElement): void {
    ...
    this.originalSelect = container.querySelector("#originalSelect") as any;
  ...
  private _onChange(event: Event): void {
    ...
    if (target.value !== ADD_CUSTOM_FONT_OPTION) {
      this.api.triggerValueChange(target.value);
    } else if (!this.dialog) {
      this._showDialog();
    }
  }
  ...
  private _showDialog(): void {
    ...
    this.dialog
      ...
      .addEventListener("click", () => this._submitDialog());
    this.dialog
    ...
    inputs.forEach((input) => {
      input.addEventListener("input", () => {
        const errorMsg = this.dialog!.querySelector(
          "#error-message"
        ) as HTMLElement;
        if (errorMsg) {
  ...
  private _getDialogTemplate(): string {
    return `<div style="
        position: fixed;
        top: 0;
        left: 0;
  ...
  private _submitDialog(): void {
    const nameInput = this.dialog!.querySelector("#name") as HTMLInputElement;
    const fontFamilyInput = this.dialog!.querySelector(
      "#fontFamily"
    ) as HTMLInputElement;
    const urlInput = this.dialog!.querySelector("#url") as HTMLInputElement;
