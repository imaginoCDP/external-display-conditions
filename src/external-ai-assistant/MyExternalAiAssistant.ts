...
export class MyExternalAiAssistant extends ExternalAiAssistant {
  ...
  openAiAssistant({
    ...
    const originalTextDiv = this.externalAiAssistant.querySelector(
      "#originalText"
    ) as HTMLElement;
    originalTextDiv.textContent = this.originalText || "No text provided";

    // Set the textarea value
    const textarea = this.externalAiAssistant.querySelector(
      "#text"
  ...
  private createModal(): void {
    ...
    this.externalAiAssistant = document.getElementById(
      "externalAiAssistant"
    ) as HTMLElement;
  ...
  private generateModalHTML(): string {
    return `
        <div id="externalAiAssistant" style="${this.styleObjToString(
          MyExternalAiAssistant.STYLES.overlay
        )}">
            <div style="${this.styleObjToString(
              MyExternalAiAssistant.STYLES.modal
            )}">
  ...
  private generateHeaderHTML(): string {
    return `
        <div style="${this.styleObjToString(
          MyExternalAiAssistant.STYLES.header
        )}">
  ...
  private generateBodyHTML(): string {
    return `
        <div style="${this.styleObjToString(
          MyExternalAiAssistant.STYLES.body
        )}">
  ...
  private generateActionButtons(): string {
    return Object.entries(MyExternalAiAssistant.TEXT_TRANSFORMATIONS)
      .map(
        ([action, config]) => `
        <button class="suggestion-btn" data-action="${action}"
            style="${this.styleObjToString(
              MyExternalAiAssistant.STYLES.suggestionButton
            )}"
  ...
  private generateFooterHTML(): string {
    return `
        <div style="${this.styleObjToString(
          MyExternalAiAssistant.STYLES.footer
        )}">
            <button class="cancelButton" style="${this.styleObjToString(
              MyExternalAiAssistant.STYLES.cancelButton
            )}"
            ...
            <button class="okButton" style="${this.styleObjToString(
              MyExternalAiAssistant.STYLES.primaryButton
            )}"
  ...
  private generateDisclaimerFooterHTML(): string {
    return `
        <div style="${this.styleObjToString(
          MyExternalAiAssistant.STYLES.disclaimerFooter
        )}">
  ...
  private attachEventListeners(): void {
    ...
    const suggestionButtons =
      this.externalAiAssistant.querySelectorAll(".suggestion-btn");
